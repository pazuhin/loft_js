...
function collectDOMStat(root) {
    ...
    function getInfo(root) {
        const childElements = [...root.childNodes];
        ...
        if (childElements.length) {
            childElements.forEach(function (elem) {
                if (elem.nodeType === 3) {
                    ...
                    (elem.tagName in res.tags) ? res.tags[elem.tagName]++ : res.tags[elem.tagName] = 1;
                    for (let cl of elem.classList) {
                        (cl in res.classes) ? res.classes[cl]++ : res.classes[cl] = 1;
                    }
                }
